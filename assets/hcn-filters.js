...
const init = () => {
    ...
    // Hardcoded policy chips — write into features (pet-friendly, allows-parties)
    const POLICY_CHIPS = [
        { slug: 'pet-friendly',   name: 'Pet friendly' },
        { slug: 'allows-parties', name: 'Allows Parties' },
    ];
    const polHardSection = form.querySelector('[data-hcn-pol-hard-section]');
    const polHardChips   = form.querySelector('[data-hcn-pol-hard-chips]');

    const renderHardPolChips = () => {
        if (!polHardChips) return;
        polHardChips.innerHTML = POLICY_CHIPS.map(p => {
            const on = selectedFeatures.has(p.slug);
            return `<button type="button" class="hcn-chip${on?' is-on':''}" data-hard-pol-slug="${p.slug}">${p.name}</button>`;
        }).join('');
        polHardChips.querySelectorAll('[data-hard-pol-slug]').forEach(btn => btn.addEventListener('click', () => {
            const slug = btn.getAttribute('data-hard-pol-slug');
            if (!slug) return;
            if (selectedFeatures.has(slug)) selectedFeatures.delete(slug); else selectedFeatures.add(slug);
            if (inFeatures) inFeatures.value = [...selectedFeatures].join(',');
            renderHardPolChips(); renderFeatChips(); updateDot();
        }));
    };

    // Policy chips (dynamic, from Settings)
    ...
    clearBtn?.addEventListener('click', e => {
        ...
        POLICY_CHIPS.forEach(p => selectedFeatures.delete(p.slug));
        ...
        setPriceUI(b.min, b.max); updateRoomUI(); renderFeatChips(); renderHardPolChips(); renderPolChips(); syncUrl();
    ...
    updateRoomUI(); renderFeatChips(); renderHardPolChips(); renderPolChips(); updateDot();
